...
import { GEMINI_HOST } from "../dist/types.js";
...
const FIXTURES = [
  ...
  ["AfterTool", { tool_name: "run_shell_command", tool_input: { command: "npm run lint" }, tool_response: { exitCode: 1 } }],
  ["AfterTool", { tool_name: "web_fetch", tool_response: { error: "timeout" } }],
...
function emitted() {
  return FIXTURES.flatMap(([hook, input, turnMs]) => mapGeminiEvent(hook, { hook_event_name: hook, ...input }, turnMs));
}
...
test("context occupancy, when reported, rides a registered unit-bearing key as a fraction", () => {
  // PreCompress carries no occupancy figure, so no reading is expected; the
  // assertion still runs so a future branch cannot improvise a spelling.
...
test("the built CLI puts the UTC offset and an idempotency key on every wire payload", () => {
  ...
  // Gemini names the hook on stdin, so no argv — one command serves every hook.
  const result = spawnSync("node", [CLI], {
    input: JSON.stringify({ hook_event_name: hook, session_id: "gm-1", ...input }),
    encoding: "utf8",
    env: {
      ...process.env,
      HOME: home,
