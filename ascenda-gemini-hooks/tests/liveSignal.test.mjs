...
const ARGV = () => [];
const HOOK_NAME_ON_STDIN = (hook) => ({ hook_event_name: hook });
const SESSION_ID = "gm-1";
const SESSION_ON_STDIN = { session_id: SESSION_ID };
const PROMPT_HOOK = "BeforeAgent";
const promptPayload = (prompt) => ({ prompt });
const SILENT_HOOK = ["BeforeModel", {}];

const FIXTURES = [
  ["SessionStart", {}],
  ["SessionEnd", {}],
  ["BeforeAgent", { prompt: "add pagination" }],
  ["BeforeModel", {}],
  ["AfterModel", {}],
  ["BeforeToolSelection", {}],
  ["BeforeTool", { tool_name: "run_shell_command" }],
  ["AfterTool", { tool_name: "run_shell_command", tool_response: { exitCode: 0 } }],
...
test("the mapping, hook by hook", () => {
  assert.equal(liveSignalFor("BeforeAgent", { prompt: "x" }).event, "prompt_submitted");
  // The leading edge carries the cadence, so the gauge rises as the agent
  // starts rather than after it finishes.
  assert.equal(liveSignalFor("BeforeTool", { tool_name: "run_shell_command" }).event, "tool_call");
  assert.equal(liveSignalFor("PreCompress", {}).event, "compaction");
  assert.equal(liveSignalFor("AfterAgent", {}).event, "stop");
});

test("AfterTool speaks only for a failure — Gemini has no separate failure hook", () => {
  assert.equal(liveSignalFor("AfterTool", { tool_response: { exitCode: 1 } }).event, "tool_failure");
  assert.equal(liveSignalFor("AfterTool", { tool_response: { error: "timeout" } }).event, "tool_failure");
  // A success here would double-count the call BeforeTool already reported.
  assert.equal(liveSignalFor("AfterTool", { tool_response: { exitCode: 0 } }), undefined);
