...
import { ASCENDA_TOOL_TYPE, TOKEN_RENEW_LEAD_MS } from "./types";
...
export class PairingService {
  ...
  async connect(): Promise<void> {
    ...
    const pairing = await vscode.window.withProgress(
      ...
      async () => this.api.createPairingSession(toolInstallationId, this.getDisplayName())
    );
  ...
  async disconnect(): Promise<void> {
    ...
    vscode.window.showInformationMessage("Ascenda disconnected from this VS Code installation.");
  }
  ...
  async getOrCreateToolInstallationId(): Promise<string> {
    ...
    const id = `${ASCENDA_TOOL_TYPE}:${crypto.randomUUID()}`;
  ...
  private getDisplayName(): string { return `${vscode.workspace.name ?? "VS Code"} on VS Code`; }
}
