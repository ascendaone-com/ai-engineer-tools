...
export class PairingPanel {
  ...
  static createOrShow(extensionUri: vscode.Uri, pairing: PairingSessionResponse): PairingPanel {
    if (PairingPanel.currentPanel) {
      PairingPanel.currentPanel.update(pairing);
      PairingPanel.currentPanel.panel.reveal(vscode.ViewColumn.One);
      return PairingPanel.currentPanel;
    }
    ...
    PairingPanel.currentPanel = new PairingPanel(panel, extensionUri, pairing);
    return PairingPanel.currentPanel;
  }

  private constructor(panel: vscode.WebviewPanel, private readonly extensionUri: vscode.Uri, pairing: PairingSessionResponse) {
    this.panel = panel;
    this.update(pairing);
  ...
  showPaired(): void { this.panel.webview.html = `<!DOCTYPE html><html><body style="font-family:sans-serif;padding:24px;"><h1>Ascenda connected</h1><p>This VS Code installation is now paired to your Ascenda app.</p><p>No email address or personal identity was shared with VS Code.</p><p>Privacy-safe workload telemetry can now be routed through the backend to your app.</p></body></html>`; }
  ...
  private getHtml(pairing: PairingSessionResponse): string {
    ...
    return `<!DOCTYPE html><html><head><meta charset="UTF-8" /><style>body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;padding:24px;line-height:1.5}.card{max-width:560px;border:1px solid var(--vscode-panel-border);border-radius:8px;padding:24px}.qr{width:240px;height:240px;margin:16px 0;background:white;padding:12px;border-radius:8px}.code{font-size:32px;letter-spacing:4px;font-weight:700;margin:12px 0}.muted{opacity:.75}</style></head><body><div class="card"><h1>Connect Ascenda</h1><p>Open the Ascenda app and scan this QR code.</p><img class="qr" src="${qrImageUrl}" alt="Ascenda pairing QR code" /><p>Or enter this pairing code in the app:</p><div class="code">${escapeHtml(pairing.code)}</div><p class="muted">Expires at ${escapeHtml(expiry)}.</p><hr /><p class="muted">This does not share your email, name, source code, prompts, files, branch names, repository names, or mobile push token. It links this VS Code installation to your Ascenda app so privacy-safe workload signals can be routed to your device.</p></div></body></html>`;
  }
}
