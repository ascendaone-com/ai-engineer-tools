...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  const api = new AscendaApi();
  const pairingService = new PairingService(context, api);
  ...
  context.subscriptions.push(vscode.commands.registerCommand("ascenda.sendTestSignal", async () => { telemetryService?.track("editor_activity", "low", { activity: "test_signal", message: "VS Code extension test signal", simulated: true }); await telemetryService?.flush(); vscode.window.showInformationMessage("Ascenda test signal queued."); }));
