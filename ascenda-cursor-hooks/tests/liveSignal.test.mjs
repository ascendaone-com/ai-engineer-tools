...
const SESSION_ON_STDIN = { conversation_id: SESSION_ID };
const PROMPT_HOOK = "beforeSubmitPrompt";
const promptPayload = (prompt) => ({ prompt });
const SILENT_HOOK = ["postToolUse", { tool_name: "shell" }];

const FIXTURES = [
  ["sessionStart", {}],
  ["sessionEnd", {}],
  ["beforeSubmitPrompt", { prompt: "add pagination" }],
  ["preToolUse", { tool_name: "shell" }],
  ["postToolUse", { tool_name: "shell", tool_output: JSON.stringify({ exitCode: 0 }) }],
  ["postToolUseFailure", { tool_name: "shell", is_interrupt: false }],
  ["postToolUseFailure", { tool_name: "shell", is_interrupt: true }],
...
test("the mapping, hook by hook", () => {
  assert.equal(liveSignalFor("beforeSubmitPrompt", { prompt: "x" }).event, "prompt_submitted");
  ...
  assert.equal(liveSignalFor("preCompact", { trigger: "auto" }).event, "compaction");
  assert.equal(liveSignalFor("stop", {}).event, "stop");
});

test("failure has its own hook here, so postToolUse counts nothing twice", () => {
  assert.equal(liveSignalFor("postToolUseFailure", { is_interrupt: false }).event, "tool_failure");
  assert.equal(liveSignalFor("postToolUse", { tool_output: JSON.stringify({ exitCode: 1 }) }), undefined);
  assert.equal(liveSignalFor("postToolUse", { tool_output: JSON.stringify({ exitCode: 0 }) }), undefined);
});

test("a user pressing stop is not a failure", () => {
  // The impulse means "something broke". Flashing it because someone
  // cancelled their own agent is a lie the gauge cannot walk back.
  assert.equal(liveSignalFor("postToolUseFailure", { is_interrupt: true }), undefined);
});

test("the specialised shell, MCP and file hooks stay silent", () => {
  // Views of calls preToolUse already reported — the same double-count the
  // telemetry mapper avoids by leaving them unregistered.
  for (const hook of ["beforeShellExecution", "afterShellExecution", "beforeMCPExecution", "afterMCPExecution", "beforeReadFile", "afterFileEdit", "beforeTabFileRead", "afterTabFileEdit"]) {
...
test("hooks with no gauge counterpart stay silent", () => {
  for (const hook of ["subagentStart", "subagentStop", "afterAgentResponse", "afterAgentThought", "workspaceOpen"]) {
    assert.equal(liveSignalFor(hook, {}), undefined, `${hook} must stay silent`);
  }
});

test("this adapter is distinct from the Cursor extension's stream", () => {
  // The extension reports `cursor_mcp`; these hooks report `cursor`. Someone
  // running both is counted twice, deliberately — see liveSignal.ts. What
  // must never happen is these two collapsing into one name, because then a
  // single install would be indistinguishable from a double one.
  assert.equal(HOST, "cursor");
  assert.notEqual(HOST, "cursor_mcp");
});

test("a prompt-less beforeSubmitPrompt omits the bucket rather than inventing one", () => {
  assert.deepEqual(liveSignalFor("beforeSubmitPrompt", {}), { event: "prompt_submitted" });
});
